...
class _Image(object):
    ...
    def close(self):
        ...
        global_handle_free(self._handle)
        self._handle = None
        
    def to_file(self, filepath):
        """Saves in-memory image to BMP file"""
        _dib_write(self._handle, filepath, _GlobalLock, _GlobalUnlock)
    
    def to_bytes(self):
        ''' convert to bytes'''
        return _dib_write(self._handle,None,_GlobalLock, _GlobalUnlock)
...
class SourceManager(object):
    ...
    def source_list(self):
        ...
        return list(self.product_list.keys())

    @property
    def product_list(self):
        '''Returns a list containing the products of the available source.'''
        names = []
        products = []
        ds_id = TW_IDENTITY()
        rv = self._call(None,
        ...
        while (rv != TWRC_ENDOFLIST):
            ...
            rv = self._call(None,
                            ...
                            byref(ds_id),
                            (TWRC_SUCCESS, TWRC_ENDOFLIST))
        return dict(zip(names,products))
